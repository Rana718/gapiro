...
<div class="flex flex-col h-full bg-surface rounded-md border border-border-subtle overflow-hidden">
  ...
  {:else}
    <div class="flex flex-col h-full">
      ...
      {#if protocol==='websocket'}<div class="flex-1 p-3 flex flex-col gap-3"><div class="rounded-md border border-border-subtle bg-surface-inset p-3 text-xs text-text-subtle">{connected?'Connected and ready for messages':'Enter a WebSocket URL to begin'}</div><textarea bind:value={message} placeholder="Message payload…" class="flex-1 p-3 rounded-md bg-surface-inset border border-border font-mono text-xs resize-none"></textarea><button class="self-end px-4 py-2 rounded-md bg-primary text-white" onclick={()=>message='' }>Send message</button></div>
      ...
      {:else}<div class="flex-1 p-3"><textarea placeholder="SELECT * FROM users LIMIT 50;" class="w-full h-full p-3 rounded-md bg-surface-inset border border-border font-mono text-xs resize-none"></textarea></div>{/if}
    </div>
  {/if}
</div>
